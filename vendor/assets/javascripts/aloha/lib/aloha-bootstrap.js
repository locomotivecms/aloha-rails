...
commands["delete"] = {
	action: function(value, range) {
		...
		// start (node, offset â 1) and end (node, offset) and abort these
		// steps."
		if (node.nodeType == $_.Node.TEXT_NODE
		&& offset != 0) {
			range.setStart(node, offset);
		...
		// "If node has a child with index offset â 1 and that child is a br or
		// hr or img, call collapse(node, offset) on the Selection. Then delete
		// the contents of the range with start (node, offset â 1) and end
		...
		&& (isHtmlElement(startNode.childNodes[startOffset - 1], "hr")
			...
			// offset â 1) and end (start node, start offset)."
		...
		// node to its child with index start offset â 1, then set start offset
...
define('aloha/repositorymanager',[
	...
	Aloha.RepositoryManager = Class.extend( {
		 ...
		 * <li><code> orderBy</code> : array  (optional) <div class="sub-desc">ex. [{lastModificationDate:âDESCâ, name:âASCâ}]</div></li>
